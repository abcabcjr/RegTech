...
type SimpleChecklistStatus struct {
	Key         string    `json:"key"`    // Format: "global:{itemId}", "asset:{assetId}:{itemId}", or "business_unit:{businessUnitId}:{itemId}"
...
func BusinessUnitChecklistKey(businessUnitId, itemId string) string {
	return "business_unit:" + businessUnitId + ":" + itemId
}
